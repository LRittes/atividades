...
{ int result;
  ...
  if ((novoNoFila = (struct noFila *) malloc(sizeof(struct noFila))) != NULL)
    { 
	  memcpy(&(novoNoFila->dados),pInfo, p->tamInfo); // novoNoFila->dados = *pInfo;
  	  if(p->frente == NULL && p->cauda == NULL) /*fila vazia*/
    	 ...
    	 { 
		   visitado = p->frente; /*maior prioridade na frente */
       	   while(visitado->atras != NULL && (visitado->dados.idade >= pInfo->idade)) 
  				visitado= visitado->atras; 

           
       	   if(visitado->dados.idade < pInfo->idade)   /* novo item fica a frente do visitado */
         	   {   	
